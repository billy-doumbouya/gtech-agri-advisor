...
import { getAgronomicAdvice, getDailyTip, diagnoseCropImage } from '../services/gemini.service.js';
...
const DAILY_TIP_KEYWORDS = ['conseil', 'conseil du jour', 'meteo', 'météo', '🌾'];
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (value?.statuses) {
      return; // à logger/traiter si besoin (ex: suivi de délivrabilité)
    }
    ...
    switch (message.type) {
      ...
      case 'interactive': {
        ...
        console.log('[whatsapp webhook] Message interactif reçu, non traité spécifiquement');
        break;
      }

      default:
        console.log(`[whatsapp webhook] Type de message non géré: ${message.type}`);
    }
  } catch (err) {
...
async function handleTextMessage(from, rawText) {
  ...
  if (!farmer.mainCrop) {
    ...
    await farmer.save();
    await sendTextMessage(from, `Merci ! Je note que tu cultives : ${farmer.mainCrop}.`);
...
async function handleImageMessage(from, buffer, mimeType) {
  const farmer = await getOrCreateFarmer(from);
  const diagnosis = await diagnoseCropImage(buffer, mimeType, { mainCrop: farmer.mainCrop });
...
async function handleAudioMessage(from, buffer, mimeType) {
  ...
  console.log(`[audio] ${from}, type: ${mimeType} — transcription non encore branchée`);
